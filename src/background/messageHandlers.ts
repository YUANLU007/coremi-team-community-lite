...
import type { ExternalModelConfig, GroupChat, GroupMessage, GroupRole, MessageAttachment, MessageReference, OpenTeamStore, RuntimeFrameBinding } from '../group/types'
...
export function createMessageHandlers(deps: MessageHandlersDependencies): BackgroundMessageRoute[] {
  ...
  const handleMessageSend = async (message: RuntimeMessage) => {
    ...
    const { store, result } = await mutateStore(store => {
      ...
      const attachments = readMessageAttachments(message.attachments, timestamp)
...
function readMessageAttachments(raw: unknown, timestamp: number): MessageAttachment[] | undefined {
  if (!Array.isArray(raw)) return undefined
  const attachments = raw.map(readMessageAttachment).filter((attachment): attachment is MessageAttachment => Boolean(attachment))
  if (attachments.length === 0) return undefined
  return attachments.slice(0, 6).map(attachment => ({
    ...attachment,
    createdAt: Number.isFinite(attachment.createdAt) ? attachment.createdAt : timestamp,
  }))
}

function readMessageAttachment(raw: unknown): MessageAttachment | undefined {
  if (!raw || typeof raw !== 'object') return undefined
  const record = raw as Record<string, unknown>
  const name = readOptionalString(record.name)?.slice(0, 180)
  if (!name) return undefined
  const mimeType = readOptionalString(record.mimeType)?.slice(0, 120) || 'application/octet-stream'
  const size = typeof record.size === 'number' && Number.isFinite(record.size) ? Math.max(0, Math.floor(record.size)) : 0
  const rawKind = record.kind
  const kind: MessageAttachment['kind'] = rawKind === 'image' || rawKind === 'text' || rawKind === 'pdf' || rawKind === 'document' || rawKind === 'video' || rawKind === 'file' ? rawKind : 'file'
  const dataUrl = readOptionalString(record.dataUrl)
  const textPreview = readOptionalString(record.textPreview)
  return {
    id: readOptionalString(record.id)?.slice(0, 80) || `att-${Math.random().toString(36).slice(2, 10)}`,
    name,
    mimeType,
    size,
    kind,
    dataUrl: kind === 'image' && dataUrl?.startsWith('data:image/') ? dataUrl : undefined,
    textPreview: kind === 'text' || kind === 'pdf' || kind === 'document' ? textPreview?.slice(0, 120_000) : undefined,
