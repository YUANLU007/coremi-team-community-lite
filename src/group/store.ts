...
export function repairStoreReferences(store: OpenTeamStore): OpenTeamStore {
  store.chatOrder = uniqueStrings(store.chatOrder).filter(chatId => Boolean(store.chatsById[chatId]))
  store.roleTemplateOrder = uniqueStrings(store.roleTemplateOrder).filter(templateId => Boolean(store.roleTemplatesById[templateId]))
  store.orchestrationFlowsById ??= {}
  store.orchestrationFlowOrderByChatId ??= {}
  store.orchestrationRunsById ??= {}
  store.activeOrchestrationRunIdByChatId ??= {}
  store.chatNotesById ??= {}
  ...
  for (const [chatId, chat] of Object.entries(store.chatsById)) {
    ...
    chat.roleIds = uniqueStrings(chat.roleIds).filter(roleId => store.rolesById[roleId]?.chatId === chat.id)
    chat.messageIds = uniqueStrings(chat.messageIds).filter(messageId => store.messagesById[messageId]?.chatId === chat.id)
    if (!store.chatOrder.includes(chat.id)) store.chatOrder.push(chat.id)
  }

  for (const [roleId, role] of Object.entries(store.rolesById)) {
    if (!role || role.id !== roleId || !store.chatsById[role.chatId]) {
      delete store.rolesById[roleId]
    }
  }

  for (const [messageId, message] of Object.entries(store.messagesById)) {
    if (!message || message.id !== messageId || !store.chatsById[message.chatId]) {
      delete store.messagesById[messageId]
    }
  }

  for (const chat of Object.values(store.chatsById)) {
    chat.roleIds = uniqueStrings(chat.roleIds).filter(roleId => store.rolesById[roleId]?.chatId === chat.id)
    chat.messageIds = uniqueStrings(chat.messageIds).filter(messageId => store.messagesById[messageId]?.chatId === chat.id)
  }

  if (store.viewState.chatReadSeqById) {
    for (const chatId of Object.keys(store.viewState.chatReadSeqById)) {
      if (!store.chatsById[chatId]) delete store.viewState.chatReadSeqById[chatId]
    }
  }
  if (store.viewState.chatHasNewMessageById) {
    for (const chatId of Object.keys(store.viewState.chatHasNewMessageById)) {
      if (!store.chatsById[chatId]) delete store.viewState.chatHasNewMessageById[chatId]
    }
  }
...
function normalizeStore(raw: unknown): OpenTeamStore {
  ...
  return repairStoreReferences(normalized)
}
...
async function loadV2Store(rawMeta: unknown): Promise<OpenTeamStore> {
  ...
  return normalizeStore(repairStoreReferences(store))
}
