...
describe('team.html chat creation UI', () => {
  it('starts directly without a local invite-code activation gate', () => {
    ...
    expect(html).not.toContain('id="invite-gate"')
    expect(html).not.toContain('id="invite-code-form"')
    expect(html).not.toContain('id="invite-code-input"')
    expect(html).not.toContain('id="invite-code-status"')
    expect(html).not.toContain('id="activate-invite-code"')
  ...
  it('renders compact icon actions for assistant messages', () => {
    ...
    expect(source).toContain('navigator.clipboard.writeText(`${message.content}${attachmentText}`)')
  ...
  it('allows the composer attachment picker to select general files', () => {
    const html = readTeamDocument()
    const composerSource = readFileSync(resolve(process.cwd(), 'src/teamPage/composerView.ts'), 'utf8')
    const backgroundSource = readFileSync(resolve(process.cwd(), 'src/background/messageHandlers.ts'), 'utf8')

    expect(html).toContain('id="attachment-input" type="file" multiple hidden')
    expect(html).not.toContain('accept="image/*')
    expect(composerSource).not.toContain('MAX_ATTACHMENT_BYTES')
    expect(composerSource).toContain("import { extractAttachmentText } from './attachmentParsers'")
    expect(composerSource).toContain('MAX_PARSED_ATTACHMENT_BYTES')
    expect(composerSource).toContain("if (file.type.startsWith('video/')) return 'video'")
    expect(composerSource).toContain("if (file.type === 'application/pdf' || /\\.pdf$/i.test(file.name)) return 'pdf'")
    expect(composerSource).toContain("if (kind === 'pdf') return 'PDF'")
    expect(composerSource).toContain("if (kind === 'document') return 'DOC'")
    expect(composerSource).toContain("if (kind === 'video') return 'VID'")
    expect(backgroundSource).toContain("rawKind === 'pdf'")
    expect(backgroundSource).toContain("rawKind === 'document'")
    expect(backgroundSource).toContain("rawKind === 'video'")
    expect(backgroundSource).toContain("kind === 'text' || kind === 'pdf' || kind === 'document'")
  })
