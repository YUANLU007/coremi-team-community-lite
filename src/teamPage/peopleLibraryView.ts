...
export function createPeopleLibraryView(deps: PeopleLibraryViewDependencies): PeopleLibraryView {
  function renderTemplates(): void {
    ...
    deps.peopleLibrarySummaryEl.textContent = `${templates.length} people`
    deps.roleTemplateSelectEl.replaceChildren(new Option('Manual person (no library template)', ''))
    ...
    if (templates.length === 0) {
      deps.peopleLibraryListEl.append(deps.emptyCard(
        deps.state.peopleLibrarySearchQuery.trim() ? `No matching ${deps.state.peopleLibraryTemplateType === 'builtin' ? 'built-in' : 'custom'} people` : `No ${deps.state.peopleLibraryTemplateType === 'builtin' ? 'built-in' : 'custom'} people yet`,
  ...
  function renderTemplateEditor(): void {
    ...
    deps.templateFormTitleEl.textContent = selectedTemplate ? `Edit person: ${selectedTemplate.name}` : 'New person'
    ...
    deps.generateTemplatePersonaEl.textContent = 'Generate with AI'
  ...
  function templateCard(template: RoleTemplate): HTMLElement {
    ...
    used.textContent = template.type === 'builtin' ? 'Built-in' : 'Custom'
    row.append(name, used)

    const description = document.createElement('div')
    description.className = 'template-description'
    description.textContent = template.description || 'No library description'
    ...
    site.textContent = `Default model: ${templateModelLabel(template, store)}${defaultChatSite === 'chatgpt' && template.chatGptGptsUrl ? ' · GPTs' : ''}`
  ...
  function renderPeopleLibraryPagination(total: number): void {
    ...
    const previous = paginationButton('Previous', deps.state.peopleLibraryPage === 0, () => {
      deps.state.peopleLibraryPage = Math.max(0, deps.state.peopleLibraryPage - 1)
      renderTemplates()
    })
    const next = paginationButton('Next', deps.state.peopleLibraryPage >= pageCount - 1, () => {
  ...
  function renderAddPersonDialog(): void {
    ...
    if (items.length === 0) {
      deps.addLibraryPeopleListEl.append(deps.emptyCard(
        deps.state.addPersonSearchQuery.trim() ? `No matching ${deps.state.addPersonTemplateType === 'builtin' ? 'built-in' : 'custom'} people` : `No ${deps.state.addPersonTemplateType === 'builtin' ? 'built-in' : 'custom'} people yet`,
        addPersonEmptyBody(),
      ))
      return
    }
    ...
    for (const item of items) {
      ...
      const site = document.createElement('div')
      site.className = 'template-description'
      site.textContent = item.chatSites.length === 0 ? 'All available sites already added' : item.source === 'temporary' ? 'Temporary person' : addPersonMetaText(item)
  ...
  function peopleLibraryEmptyBody(): string {
    if (deps.state.peopleLibraryCategory !== TEMPLATE_CATEGORY_ALL) return `No ${deps.state.peopleLibraryTemplateType === 'builtin' ? 'built-in' : 'custom'} people in this category`
    return deps.state.peopleLibraryTemplateType === 'builtin' ? 'Switch to custom people or adjust your search.' : 'Create a person from the top-right button; saved people will appear here.'
  }
  ...
  function openBuiltinTemplateDetail(template: RoleTemplate): void {
    ...
    deps.builtinTemplateDetailMetaEl.textContent = `${templateMetaText(template)} · Default model: ${templateModelLabel(template, deps.getStore())}`
    deps.builtinTemplateDetailPromptEl.textContent = template.systemPrompt || 'No prompt provided'
  ...
  function addPersonEmptyBody(): string {
    if (deps.state.addPersonCategory !== TEMPLATE_CATEGORY_ALL) return `No ${deps.state.addPersonTemplateType === 'builtin' ? 'built-in' : 'custom'} people in this category`
    return deps.state.addPersonTemplateType === 'builtin' ? 'Switch to custom people or adjust your search.' : 'Create a person in the library first, or use temporary add from the top-right button.'
  }
  ...
  function validatePersonDraft(draft: Pick<TemplateDraft, 'name' | 'description' | 'systemPrompt'>): string | undefined {
    if (!draft.name) return 'Person name is required'
    if (Array.from(draft.name).length > ROLE_NAME_MAX_CHARACTERS) return `Person name can be at most ${ROLE_NAME_MAX_CHARACTERS} characters`
    return undefined
  }
  ...
  async function generateTemplatePersona(): Promise<void> {
    ...
    if (!description) {
      deps.showError('Describe the persona you want to generate first')
      return
    }

    deps.generateTemplatePersonaEl.disabled = true
    deps.generateTemplatePersonaEl.textContent = 'Generating'
    deps.templatePersonaGenerationStatusEl.textContent = 'Generating...'
    try {
      ...
      deps.templatePersonaGenerationStatusEl.textContent = 'Generated. You can edit and save it.'
    } catch (error) {
      deps.templatePersonaGenerationStatusEl.textContent = ''
      throw error
    } finally {
  ...
  function renderExternalModelSelect(): void {
    ...
    if (models.length === 0) {
      deps.templateExternalModelSelectEl.append(new Option('Add an external model in settings first', ''))
  ...
  async function addPeopleToCurrentChat(items: Record<string, unknown>[]): Promise<void> {
    const chat = deps.getCurrentChat()
    if (!chat) return
    if (items.length === 0) throw new Error('Select or fill in people to add')
  ...
  function deleteTemplate(template: RoleTemplate): void {
    ...
    if (!window.confirm(`Delete "${template.name}" from the people library?`)) return
...
function templateTypeLabel(type: RoleTemplate['type']): string {
  return type === 'builtin' ? 'Built-in person' : 'Custom person'
}
...
function personaGenerationErrorMessage(error: unknown): string {
  ...
  if (message === 'Unknown OpenTeam message') return 'Reload the Coremi Team extension before using AI persona generation'
  return message
}
